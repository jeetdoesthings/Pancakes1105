...
class Orchestrator:
    ...
    async def _node_junior_analyst(self, state: GraphState, config: RunnableConfig):
        ...
        # Construct UniversalRFP as canonical intermediate format from extracted requirements
        from app.rfp_schema import UniversalRFP
        from datetime import date
        try:
            deadline_str = requirements.response_deadline
            deadline_date = date.fromisoformat(deadline_str) if deadline_str else date.today()
        ...
        await emit(AgentMessage(
            ...
            content=f"✅ UniversalRFP schema populated: rfpId={universal_rfp.rfpId}, title='{universal_rfp.title}', "
                    f"products='{universal_rfp.productName[:60]}...', currency={universal_rfp.currency}"
        ))
        ...
        if similar_rfps:
            await emit(AgentMessage(
                agent=AgentRole.ORCHESTRATOR,
                message_type=MessageType.RESULT,
                content=f"🔍 Found {len(similar_rfps)} similar historical RFPs via hybrid FTS5 + semantic search"
            ))
            for i, sim in enumerate(similar_rfps):
                rfp = sim["rfp"]
                score = sim["combined_score"]
                await emit(AgentMessage(
                    agent=AgentRole.ORCHESTRATOR,
                    message_type=MessageType.ACTION,
                    content=f"  [{i+1}] '{rfp.title}' (score: {score:.2f}) — {rfp.productName[:50]} — Budget: {rfp.currency} {rfp.budget:,.0f}" if rfp.budget else f"  [{i+1}] '{rfp.title}' (score: {score:.2f}) — {rfp.productName[:50]} — Budget: N/A"
                ))
        else:
            await emit(AgentMessage(
                agent=AgentRole.ORCHESTRATOR,
                message_type=MessageType.STATUS,
                content="🔍 No similar historical RFPs found in database. Proceeding with fresh analysis."
            ))

        # Index this RFP for future retrieval
        rfp_text = state.get("rfp_input", {}).rfp_text if state.get("rfp_input") else ""
        rfp_similarity_service.index_rfp(universal_rfp, rfp_text)

        return {
            "extracted_requirements": requirements,
            "universal_rfp": universal_rfp.model_dump(),
            "similar_rfps": [
                {
                    "rfp": s["rfp"].model_dump(),
                    "fts_score": s["fts_score"],
                    "semantic_score": s["semantic_score"],
                    "combined_score": s["combined_score"],
                }
                for s in similar_rfps
            ],
    ...
    async def _node_pricing_strategist(self, state: GraphState, config: RunnableConfig):
        ...
        strategy = await self.pricing_strategist.analyze(
            ...
            emit_message=emit,
            additional_instructions=instructions,
            similar_rfps=state.get("similar_rfps", []),
        )
    ...
    async def _node_pdf_generator(self, state: GraphState, config: RunnableConfig):
        ...
        try:
            pdf_path = await asyncio.to_thread(
                ...
                universal_rfp=state.get("universal_rfp"),
                similar_rfps=state.get("similar_rfps", []),
            )
